...
use crate::dao::{insert_encrypted_message, copy_message_as_forward, mark_read, mark_delivered, recall_message, EncryptedMessageRecord};
...
impl msgpb::friend_msg_service_server::FriendMsgService for Services {
    async fn send_message(&self, request: Request<msgpb::Content>) -> Result<Response<()>, Status> {
        ...
        if shard != self.shard_index() { return Ok(Response::new(())); }

        // 好友关系校验
        if let Some(cli) = self.friend_client() {
            let req = IsFriendReq { user_id: content.sender_id, friend_id: content.receiver_id };
            if let Ok(resp) = cli.clone().is_friend(req).await {
                if !resp.into_inner().is_friend { return Err(Status::permission_denied("not friends")); }
            }
        }
            ...
            if let Some(first) = content.contents.first() {
                if let Some(msgpb::message_content::Content::Encrypted(enc)) = &first.content {
                    ...
                    let rec = EncryptedMessageRecord {
                        ...
                        aad: if enc.aad.is_empty() { None } else { Some(enc.aad.clone()) },
        ...
        if let Some(kafka) = self.kafka() {
            let _ = kafka
                .send_proto(100, &content, &(content.message_id.unwrap_or_default() as i64), &MSG_SEND_FRIEND_TOPIC.topic_name)
                .await;
        }
        Ok(Response::new(()))
    }

    async fn report_msg_read(&self, request: Request<msgpb::MsgRead>) -> Result<Response<()>, Status> {
        let r = request.into_inner();
        let _ = mark_read(self.pool(), r.msg_id, r.read_at).await.map_err(|e| Status::internal(format!("db error: {e}")))?;
        Ok(Response::new(()))
    }

    async fn ack_msg_delivered(&self, request: Request<msgpb::MsgDeliveredAck>) -> Result<Response<()>, Status> {
        let r = request.into_inner();
        let now = chrono::Utc::now().timestamp_millis();
        let _ = mark_delivered(self.pool(), r.msg_id, now).await.map_err(|e| Status::internal(format!("db error: {e}")))?;
        Ok(Response::new(()))
    }

    async fn ack_msg_read(&self, _request: Request<msgpb::MsgReadAck>) -> Result<Response<()>, Status> {
        Ok(Response::new(()))
    }
    ...
    async fn forward_msg(&self, request: Request<msgpb::MsgForward>) -> Result<Response<()>, Status> {
        let r = request.into_inner();
        copy_message_as_forward(self.pool(), r.src_msg_id, r.new_msg_id.unwrap_or_default(), r.from_user_id, r.to_user_id, r.created_at)
            .await
            .map_err(|e| Status::internal(format!("db error: {e}")))?;
        // Kafka 通知新消息
        if let Some(kafka) = self.kafka() {
            ...
            let _ = kafka
                .send_proto(100, &c, &(c.message_id.unwrap_or_default() as i64), &MSG_SEND_FRIEND_TOPIC.topic_name)
                .await;
        }
        Ok(Response::new(()))
    }

...
use crate::dao::{
    copy_message_as_forward, fetch_device_bundles, insert_encrypted_message, mark_delivered, mark_read,
    recall_message, DeviceKeysRow, EncryptedMessageRecord,
};
...
impl Services {
    pub fn friend_client(&self) -> Option<&HfFriendClient<tonic::transport::Channel>> { self.friend_client.as_ref() }
    pub fn pool(&self) -> &Pool<MySql> { &self.pool }
    pub fn kafka(&self) -> Option<&Arc<KafkaInstanceService>> { self.kafka.as_ref() }
    pub fn shard_index(&self) -> u32 { self.shard_index }
    pub fn shard_total(&self) -> u32 { self.shard_total }
}
...
pub async fn start_grpc_server(bind: &str) -> anyhow::Result<()> {
    ...
    let pool = get_db();
    let friend_client = if let Some(addr) = AppConfig::get().grpc.as_ref().and_then(|g| g.client_addr.clone()) {
    ...
    let shard_total: u32 = std::env::var("FRIEND_SHARD_TOTAL").ok().and_then(|s| s.parse().ok()).unwrap_or(16);
    let shard_index: u32 = std::env::var("FRIEND_SHARD_INDEX").ok().and_then(|s| s.parse().ok()).unwrap_or(0);

    let svc = Services { pool, friend_client, kafka, shard_index, shard_total };

    tonic::transport::Server::builder()
        .add_service(FriendMsgServiceServer::new(svc.clone()))
        .add_service(FriendBizServiceServer::new(MsgFriendServiceImpl::new(svc.clone().into())))
...
pub async fn run_server() -> anyhow::Result<()> {
    let app_cfg = AppConfig::init_from_env("./config-friend.toml").await;
    let grpc_addr_str = app_cfg
        .grpc
        .and_then(|g| g.server_addr)

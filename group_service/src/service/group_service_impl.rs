    ...
    S: GroupStorage + Send + Sync + 'static,
{
    ...
    async fn get_managers(
        &self,
        req: Request<GetManagersReq>,
    ) -> Result<Response<GetManagersResp>, Status> {
        let r = req.into_inner();
        let managers = self
            .facade
            .get_managers(r.group_id)
            .await
            .map_err(|e| Status::internal(format!("get_managers failed: {e}")))?;
        Ok(Response::new(GetManagersResp { managers }))
    }

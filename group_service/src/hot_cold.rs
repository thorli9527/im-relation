...
impl<S: GroupStorage> HotColdFacade<S> {
    /// 旧签名：快速构造（保留兼容）
    pub fn new(map: Arc<ShardMap>, storage: Arc<S>, hot_capacity: u64, hot_tti_secs: u64) -> Self {
        Self::with_config(
            map,
            storage,
            HotColdConfig {
    ...
    /// 推荐：使用配置构造
    pub fn with_config(map: Arc<ShardMap>, storage: Arc<S>, cfg: HotColdConfig) -> Self {

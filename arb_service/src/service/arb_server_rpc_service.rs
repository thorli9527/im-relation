...
use crate::grpc_arb::client::{connect_client, invalidate_client};
...
impl ArbServerRpcServiceImpl {
    /// 创建新的仲裁服务实例
    const NODE_TIMEOUT_MS: u64 = 60_000;
    ...
    async fn sync_to_nodes(&self, exclude_addr: &str, blob: BytesBlob) {
        ...
        for node in nodes {
            ...
            tokio::spawn(async move {
                // 解析地址
                let _uri = match Uri::try_from(&addr) {
                    Ok(uri) => uri,
                    Err(e) => {
                ...
                let result = timeout(Duration::from_secs(5), async {
                    match connect_client(&addr).await {
                        ...
                        Err(e) => {
                            ...
                            Err(Status::unavailable(format!("连接失败: {}", e)))
                        }
                    }
                })
                .await;
    ...
    fn spawn_stale_monitor(&self) {
        ...
        tokio::spawn(async move {
            let interval = ArbServerRpcServiceImpl::CLEANUP_INTERVAL;
            loop {
                ...
                let now = match SystemTime::now().duration_since(UNIX_EPOCH) {
                    ...
                    Err(e) => {
                        warn!("cleanup: system time error: {}", e);
                        continue;
                    }
                };
                ...
                for mut entry in node_list.iter_mut() {
                    let kind = *entry.key();
                    let vec = entry.value_mut();
                    let mut removed_nodes = Vec::new();
                    vec.retain(|node| {
                        let expired = now.saturating_sub(node.last_update_time)
                            > ArbServerRpcServiceImpl::NODE_TIMEOUT_MS;
                        if expired {
                    ...
                    for node in removed_nodes {
                        ...
                        info!(
                            "cleanup: stale node removed type={:?} addr={}",
                            node_type, node.node_addr
                        );
                        if node_type == NodeType::SocketNode {
                            expired_socket_nodes.push(node.clone());
                        }
                    }
                }

                for kind in empty_types {
                    node_list.remove(&kind);
                }
                ...
                for node in expired_socket_nodes {
                    match serde_json::to_vec(&node) {
                        Ok(data) => {
                            ...
                            let svc = ArbServerRpcServiceImpl {
                                node_list: node_list.clone(),
                                client_senders: client_senders.clone(),
                            };

                            if let Err(e) = svc.broadcast_to_clients(blob.clone()).await {
                                warn!(
                                    "cleanup: broadcast socket removal {} failed: {}",
                                    node.node_addr, e
                                );
                            }

                            svc.sync_to_nodes(&node.node_addr, blob).await;
                        }
                        Err(e) => warn!("cleanup: serialize node {} failed: {}", node.node_addr, e),
                    }
                }
            }
        });
...
impl ArbServerRpcService for ArbServerRpcServiceImpl {
    ...
    async fn update_shard_state(
        ...
        if let Some(mut entry) = self.node_list.get_mut(&node_type) {
            if let Some(node) = entry.iter_mut().find(|n| n.node_addr == req.node_addr) {

...
use common::grpc::grpc_hot_online::online_service::client_rpc_service_server::ClientRpcService;
use common::grpc::grpc_hot_online::online_service::{
    ...
    FindByContentReq, FindClientDto, GetClientReq, GetClientsReq, RegisterUserReq, UpdateClientReq,
};
...
#[tonic::async_trait]
impl<C, D, N, I> ClientRpcService for ClientEntityServiceImpl<C, D, N, I>
where
    C: ClientReadRepo + Send + Sync + 'static,
    D: DirectoryReadRepo + Send + Sync + 'static,
    N: Normalizer,
    I: IdAllocator,
{
    type GetClientsStream =
        Pin<Box<dyn tokio_stream::Stream<Item = Result<ClientEntity, Status>> + Send + 'static>>;
    ...
    async fn get_clients(
        &self,
        req: Request<GetClientsReq>,
    ) -> Result<Response<Self::GetClientsStream>, Status> {
        let ids = req.into_inner().ids;
        if ids.is_empty() {
            let stream = iter(std::iter::empty::<Result<ClientEntity, Status>>());
